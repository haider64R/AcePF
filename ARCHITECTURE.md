...
## AcePF V1.1 presentation and Notes depth

`src/content/notes-depth.js` enriches the ten validated Notes records before they are exported. It merges teaching blocks into existing section kinds, adds missing `details`/`worked-example` sections before Quick Revision, and leaves all 19 runnable sample IDs and source programs unchanged. The resulting guide list is ordered by the canonical taxonomy category `order`; Notes cards, curriculum map, and sidebar therefore use the same sequence. `NOTES_V11_AUDIT.md` maps the taxonomy groups to the expanded teaching content.

`src/ui/code-display.js` is a presentation-only C++ formatter. It tokenizes canonical question code, adds whitespace and line breaks for display, then accepts the result only when the ordered token text is identical. `displayFormat: "preserve"` opts a question out when its original layout is part of the task. Two binding-sensitive nested examples have explicit token-checked display layouts. Question Bank, Challenge, Exam/Past Paper/Mock attempts and review screens share `questionCodeBlock`; Visualizer handoff still receives the untouched canonical `question.code`. Internal paper records and zero-question sources remain in the content bank, while `studentPastPaperSets()` exposes only usable sets and the cards derive friendly topic/style summaries from available verified questions.
