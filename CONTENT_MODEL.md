...
`src/content/questions.js` is the single 72-record question bank: three original built-in Challenges, 29 PDF-derived questions and 40 new AcePF-authored questions. Challenge V2 and Exam Mode select from it without copying records. A record uses:
...
Supported schema type IDs are `predict-output`, `predict-value`, `predict-array-state`, `identify-error`, `multiple-choice`, `code-reasoning`, `code-writing`, `code-completion`, `short-answer`, `flowchart`, and `pseudocode`. Adding a type means extending validation and the relevant consumer UI intentionally; it does not require a second bank. The original quick-challenge dialog still selects its three tagged prediction records, while Challenge V2 selects verified auto-gradable prediction/MCQ records.
...
Add a question once to the canonical bank: the three original Challenge items remain in `questions.js`, sourced questions are in `assessment-corpus.js`, and newer authored questions are in `authored-questions.js`; both are imported into `questions.js`. Use real topic IDs and explicit provenance. For a supported executable question, run its source through `runProject` and compare with native C++17 before claiming execution verification. Tag it `challenge` only when one of the current prediction types fits the existing dialog; the dialog will pick it up through `queryQuestions`. Exam Mode, past-paper browsing and Notes links query the same bank by ID and metadata. A Visualizer action should load `code` or `files` plus `standardInput` and `virtualFiles` from that record.
...
## AcePF V1 assessment consumers

`src/learning/assessment.js` selects Challenge candidates by canonical filters and supports balanced, repeatable question sets. A session stores kind, title, question IDs, answers, position, start time, optional duration and submission time. Automatic scoring applies only to supported deterministic types with `autoGradable: true`; open writing and source-only material receive manual-review status and never affect the percentage.

Exam Mode's Question Bank can inspect all records, including two draft/source-only corpus records, with source and verification clearly shown. Past-paper attempts select only `status: "verified"` records whose source is a genuine exam; every imported document is labelled a partial question set. The Fall 2023 final has zero safely imported individual questions. Practice mocks draw only AcePF-authored, verified auto-gradable questions within the observed assessment-profile topics and selected syllabus ceiling. They are labelled authored practice, not historical papers.
