...
* Every raw event includes a detached state. Dry Run groups these into reasoning steps; Expression Details expands their recorded calculations, and Detailed Trace exposes every raw event. Previous step and mode changes only select existing snapshots. A run is recorded in a worker, not executed on demand during playback. No language features were added for educational grouping.
