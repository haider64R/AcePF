...
## AcePF V1 verification (2026-09-24)

The complete suite passed **187 tests, zero failures and zero skips**. This includes the 176-test assessment-corpus baseline, 29 new authored native-comparison cases across the 40-question AcePF expansion, Challenge V2 selection/scoring tests, Exam Mode source/timer/manual-scoring tests, and 19 native-verified Notes samples. The unchanged **35/35** curated/generated interpreter comparisons, **4/4** Dry Run acceptance comparisons and **17/17** deterministic sourced assessment comparisons still pass. All 29 executable new authored questions matched native C++17 and the Visualizer. Static JavaScript checks and the production build passed. The execution core was not changed.

The 72-record shared bank comprises 29 sourced questions and 43 authored questions, including the original three quick Challenges. Question Bank browsing shows all records with source and verification; past-paper attempts include only verified genuine exam imports. The two draft/source-only corpus records are not treated as verified paper questions. Every historical set is partial; the image-only Fall 2023 final has no individual questions in an attempt. Practice mocks use only authored, verified auto-gradable questions. A session persists its answers locally while active, uses its original start time after refresh, and excludes manual questions from computed scores.

Actual-browser QA covered Home, all main routes, Notes-to-Visualizer and Notes-to-question links, the Examples deep link and its 15-item library, Challenge setup/question/feedback, Question Bank filters, a partial past-paper attempt with an unanswered confirmation, post-submit automatic/manual scoring and Visualizer review links, and mock configuration. Active attempts displayed no answer, explanation, hint or Visualizer link. The redesigned Visualizer navigation retained the existing execution workspace; the original engine/trace interaction regressions remain covered by the unchanged tests and prior browser checks above. At measured CSS widths **1371, 1219, 731 and 371 pixels**, all five main pages had document scroll width equal to viewport width. Code and navigation scroll within their containers at narrow widths. The local browser's zoom maps requested viewport widths to those measured CSS widths. These are interactive single-browser checks, not a cross-browser or proctoring certification.

Known limits: static client-side Exam Mode is a practice tool, not a secure exam system; a technically inclined user can inspect bundled answer data. Manual/code-writing answers require human judgment. Exact-output grading is deterministic and may reject an equivalent paraphrase. Historical papers are intentionally partial and the supplied evidence does not justify a universal exam-format claim. The Visualizer remains the documented C++ teaching subset, not a full compiler.

## AcePF V1.1 Notes and presentation verification (2026-09-24)

The pre-change baseline was **187 passing tests, zero failures, zero skips**. The final suite passed **190 tests, zero failures, zero skips**. Three new display-format tests check ordered C++ token identity for every code-bearing question, canonical-source and Visualizer-handoff integrity, sensitive nested binding, and the exact-layout escape hatch. Notes tests now require canonical category order and a complete depth path for every guide. The existing native gates remained green: **35/35** curated/generated C++17 comparisons, **4/4** Dry Run acceptance comparisons, **17/17** deterministic sourced answers, **29/29** executable authored answers, and **19/19** runnable Notes samples. JavaScript syntax and static build checks passed. No lexer, parser, interpreter, event, snapshot, or playback code was changed.

Browser QA used the built application. It confirmed the ten Notes cards and sidebar in canonical order; an expanded Functions guide; a Notes sample opening unchanged source in the Visualizer and producing a 10-step/32-raw-event recording; Question Bank display for the dangling-else example; a partial Past Paper attempt, unanswered confirmation, and results/review code; Challenge question, answer feedback, and review code; and Mock Exam setup and active question code. Nine actionable paper cards appeared; the zero-question Fall 2023 final remained in the source inventory but was absent from the student catalogue. At requested viewport widths **1440, 1024, 768, and 390 CSS pixels**, the browser measured **1371, 975, 731, and 371 pixels** respectively. Notes, Exam Mode, Challenges, and Visualizer had no page-level horizontal overflow at those sizes. At phone width, Notes tables scrolled within their containers (460px content within 346px visible area), section jumps reached the worked example, and formatted code stayed inside the question card. This is a single-browser interactive check, not a cross-browser or assistive-technology certification.

## Public root route verification (2026-09-24)

The static page names were adjusted without changing the execution core: `index.html` is Home and `visualizer.html` is the Visualizer. The build emits exactly those two pages alongside `notes.html`, `practice.html`, and `exam.html`; it no longer emits `home.html`. The complete automated suite passed **192 tests, zero failures, zero skips**, including two focused source-route/deep-link tests. JavaScript syntax and the static build passed, and every relative HTML document link in the built pages resolved to an emitted file. Browser checks confirmed `/` and `/dist/index.html` display Home, Home opens `/dist/visualizer.html`, a Notes sample opens `/dist/visualizer.html?note=...`, and the Notes, Challenges, and Exam pages load from their unchanged routes. Historical checks above refer to the page names at the time they were run.
